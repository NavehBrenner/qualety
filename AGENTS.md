...
9. [docs/rulesets/fingerprint.md](docs/rulesets/fingerprint.md) — fingerprint plugin catalog (`fingerprint/hash-covers-every-config-field`, `fingerprint/versioned-hash-payload`, `fingerprint/hash-includes-code-version`).
10. [docs/rulesets/plugin-kit.md](docs/rulesets/plugin-kit.md) — portable authoring (`plugin-kit/…`); not a product app catalog.
11. [docs/rulesets/](docs/rulesets/) — research inventories are **not** an implementation backlog.
...
- Product plugins are `@qualety/typescript`, `@qualety/react`, `@qualety/dry`, `@qualety/python`, `@qualety/ml`, and `@qualety/fingerprint`. Do not re-scaffold the engine. Do not pad plugins with empty stub rules.
