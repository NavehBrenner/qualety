...
7. **Relationship to classic linters/formatters**  
    ...
    **ML plugin — what we own:** `ml/require-global-seed`, `ml/seed-must-reach-framework-rng`, `ml/dataloader-worker-seeding`, `ml/tf32-must-be-explicit`, `ml/determinism-test-required`, `ml/deterministic-algorithms-opt-in`, `ml/metadata-writer-required`, `ml/record-code-version`, `ml/run-metadata-completeness`, `ml/artifact-hash-recorded`, `ml/no-inplace-artifact-clobber`, `ml/no-refit-at-inference`, `ml/inference-mode-required`, `ml/pack-padded-sequence-before-rnn`, `ml/train-mode-restored`, `ml/optimizer-zero-grad`, `ml/tensor-to-device-result-ignored`, `ml/no-network-in-tests`, `ml/no-cuda-hardcoded`. No Ruff/Biome deltas. Does not re-provide `"python"`.

    **Fingerprint plugin — what we own:** `fingerprint/hash-covers-every-config-field`, `fingerprint/versioned-hash-payload`, `fingerprint/hash-includes-code-version`. Python-only content-addressed cache / fingerprint honesty. Not ML-specific. Does not re-provide `"python"`.
...
8. **One provider map / one engine loop**  
   ...
   **Who provides:** one map. Start empty → register every `provides` entry from loaded **plugin modules** (collision → exit 2, names **both owners**) → for each id in the **default registry** still missing, fill with `owner: "default"`. Defaults only fill gaps; they never replace a plugin-provided id.    A plugin `provides.typescript` **wins** (default skipped for that id). v1 default registry is `as const` with `"typescript"` → today’s `ParsedProject` and `"git-worktree"` → `GitWorktreeArtifact` (soft-unavailable outside git; spawn only in `provides.build`). Default providers are compiled-in trusted factories (`DEFAULT_PROVIDERS` / `createTypeScriptProvider` / `createGitWorktreeProvider`), gap-filled after plugin `provides`, and are **not** loaded through `pluginSchema`.     `dupehound` stays in `@qualety/dry`. `python` stays in `@qualety/python` (not a default). Multi-team shared providers load as **ruleless plugins** via `plugins[]`. There is **no** `config.languages` and **no** reserved-id category.
...
10. **Distribution surfaces**  
    ...
    **Official plugins** (binary bundle) are product app catalogs, not authoring or dogfood:
    ...
    | `@qualety/dry` | yes | bundled |
     ...
     | `@qualety/fingerprint` | yes | bundled |
...
- **Artifact**: Opaque value built once per check when an enabled rule `requires` its id. One provider map: plugin `provides` first, then default registry gap-fill (`"typescript"` → `ParsedProject`; `"git-worktree"` → `GitWorktreeArtifact`; `"dupehound"` and `"code-embeddings"` in `@qualety/dry`; `"python"` in `@qualety/python`). `"code-embeddings"` is a check-time artifact, not `qualety index`. Rules must not spawn git; the default `"git-worktree"` provider may spawn in `provides.build`.
...
export type GitWorktreeArtifact = {
  toplevel?: string; // absolute; omitted when unavailable
  available: boolean;
...
| `ml/metadata-writer-required` | Implemented (Gate B∨C must call a writing `writerName`, default `save_metadata`; see [ml.md](./rulesets/ml.md)) |
| `ml/record-code-version` | Implemented (writer payload must include an allowlisted code-version key; see [ml.md](./rulesets/ml.md)) |
| `ml/run-metadata-completeness` | Implemented (CLI/config dests must reach the writer payload; `allowExclusions`; see [ml.md](./rulesets/ml.md)) |
| `ml/artifact-hash-recorded` | Implemented (Gate C path/bytes-bound hash must reach writer payload; see [ml.md](./rulesets/ml.md)) |
| `ml/no-inplace-artifact-clobber` | Implemented (Gate C / run-output in-place clobber of tracked or dirty git paths; see [ml.md](./rulesets/ml.md)) |
| `ml/no-refit-at-inference` | Implemented (no transformer `.fit` / batch mean-std normalize on inference paths; see [ml.md](./rulesets/ml.md)) |
| `ml/inference-mode-required` | Implemented (inference forward needs no_grad/inference_mode and `.eval()`; see [ml.md](./rulesets/ml.md)) |
| `ml/pack-padded-sequence-before-rnn` | Implemented (pack padded sequences before RNN when `h_n` is consumed; see [ml.md](./rulesets/ml.md)) |
| `ml/train-mode-restored` | Implemented (restore `.train()` after mid-epoch `.eval()` before the next backward; see [ml.md](./rulesets/ml.md)) |
| `ml/optimizer-zero-grad` | Implemented (training backward+step must `zero_grad`; see [ml.md](./rulesets/ml.md)) |
| `ml/tensor-to-device-result-ignored` | Implemented (bare tensor `.to`/`.cuda`/dtype Expr is ignored; see [ml.md](./rulesets/ml.md)) |
| `ml/no-network-in-tests` | Implemented (test modules must not download weights; see [ml.md](./rulesets/ml.md)) |
| `ml/no-cuda-hardcoded` | Implemented (no literal CUDA device hardcoding; see [ml.md](./rulesets/ml.md)) |

Authored in TypeScript. Does not provide `"python"`; rules `requires: ["python"]` (`ml/no-inplace-artifact-clobber` also `"git-worktree"`). Imports `@qualety/python/walk` for shared AST walk helpers. No Ruff/Biome deltas.

### v1 fingerprint plugin catalog

`@qualety/fingerprint` (`name: "fingerprint"`) ships only:

| Rule | Status |
|------|--------|
| `fingerprint/hash-covers-every-config-field` | Implemented (bound hash fn must read every config field minus `excludeFields`; see [fingerprint.md](./rulesets/fingerprint.md)) |
| `fingerprint/versioned-hash-payload` | Implemented (bound hash fn must fold a version contribution; fail closed; see [fingerprint.md](./rulesets/fingerprint.md)) |
| `fingerprint/hash-includes-code-version` | Implemented (recommended `off`; allowlisted code version in hash payload; see [fingerprint.md](./rulesets/fingerprint.md)) |

Authored in TypeScript. Does not provide `"python"`; rules `requires: ["python"]`. Imports `@qualety/python/walk` for shared AST walk helpers. No Ruff/Biome deltas. Not an ml/python bag.
