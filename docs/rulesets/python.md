...
**Quiet:** all defs in `__init__.py` (barrel analog). Dunder methods `/^__\w+__$/`. Methods on a class whose non-empty `bases` all fail to resolve to an in-package `ClassDef` (stdlib / third-party / `object` / unknown; Name or `mod.Cls` only) — accepted underapprox; empty bases and any in-package base skip this arm. No `package.exports` / React port. No framework decorator allowlist (known over-flag).

**Uses:** package-local call sites (same-module `Name`; `from … import` / `import mod` + `mod.fn` resolved to a source in that package; `self.` / `cls.` methods on the enclosing class; `Name.attr(...)` whose owner is not `self`/`cls`/a known module bind — unique same-group method/property `def.name` counts that def, several same-named methods → value-load silence all of them; `Name().attr(...)` when the Call’s func Name uniquely names a group class; `self._x.attr(...)` / `cls._x.attr(...)` when `_x` is bound in that class body or `__init__` to a Name / `Name(...)` that uniquely names a group class — unproven attribute bind → value-load silence same-named methods) **and** Load-context `Name` / `mod.fn` / `self.` / `cls.` / `Name.attr` references that resolve to the def (registries, callbacks, `key=f`, `partial(f)`, assignment RHS, return, decorator, property reads; same unique / multi-match fallback). Nested `a.b.c` out. Module-level functions are not this fallback (avoids counting unresolved `mod.fn`). Import-only / re-export-only / `__all__` strings / Store / the def body do not count. `Call.func` is a call, not a value-load. ≥1 resolved value-load → silence (multiplicity through a container is unknown). Call-only ≤ 1 still reports when shape gates hold. Static `importlib.util.spec_from_file_location` / `importlib.machinery.SourceFileLoader` / `runpy.run_path` path loads in the same package group (including tests as loaders) count as `mod.fn` uses when the path folds and the alias binds; if the path is static to a group file but the alias cannot be proven, silence all defs in that target file. Unresolved / non-static dynamic is still not a counted use. Absolute imports resolve sibling of the importer, then `packageDir/…`, then `packageDir/src/…` (static `src/` only; no setuptools/hatch map); unresolved after that is not a counted use. Relative imports are unchanged. `from x import *` is a known miss.
...
**Uses:** instantiation `Call` whose callee resolves to the class; `ClassDef.bases` that resolve to the class; **and** Load-context `Name` / `mod.Cls` references that resolve to the class (registries, callbacks, passing the class as a value). Import-only / re-export-only do not count. The class’s own body does not count as external use. `Call.func` and `ClassDef.bases` are already instantiate/subclass uses, not value-loads. ≥1 resolved value-load → silence. Static `importlib.util.spec_from_file_location` / `importlib.machinery.SourceFileLoader` / `runpy.run_path` path loads in the same package group (including tests as loaders) count as `mod.Cls` uses when the path folds and the alias binds; if the path is static to a group file but the alias cannot be proven, silence all classes in that target file. Unresolved / non-static dynamic is still not a counted use. Absolute imports use the same sibling → `packageDir/…` → `packageDir/src/…` order as `python/no-unnecessary-def`.
